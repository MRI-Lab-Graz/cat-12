...
✅ **BIDS App Compliance** - Follows standard BIDS Apps specifications
✅ **Automatic Longitudinal Detection** - No manual flags needed
✅ **Modular Processing** - Opt-in stages for flexibility  
✅ **Contained Installation** - Everything in project directory using UV
✅ **No MATLAB License** - Uses CAT12 standalone
✅ **Server Optimized** - No GUI, CUDA support
...
│
├── Installation
├── install_cat12_standalone.sh        # Main installation script (uses UV)
...
│
├── Main Application
├── bids_cat12_processor.py           # Main BIDS App script
├── example_usage.py                  # Usage examples
│
├── Processing Scripts
├── scripts/
│   ├── longitudinal_template.m       # MATLAB template for CAT12
│   └── subject_processor.py          # Subject-level processing
│
├── Utilities
├── utils/
...
- `--no-cuda` - Disable GPU (enabled by default)
...
python bids_cat12_processor.py /data/bids /data/output participant --smooth-volume

# Step 3: Add QA
python bids_cat12_processor.py /data/bids /data/output participant --qa --tiv
```
...
**User Command:**
```bash
--smooth-volume --volume-fwhm "6 6 6" --smooth-prefix "s6"
```
...
**User Command:**
```bash
--smooth-surface --surface-fwhm "12"
```
...
python bids_cat12_processor.py test_data test_output participant \
