...
class CAT12ScriptGenerator:
    ...
    def generate_longitudinal_script(
...
class CAT12Processor:
    ...
    def __init__(self, config: Dict):
        ...
        self.threads_per_job = int(
            self.config.get("cat12", {}).get("threads_per_job", 0) or 0
        )
    ...
    def execute_script(
        ...
        try:
            ...
            logger.info(
                f"{Fore.YELLOW}⏳ CAT12 processing started (this may take 10-30 minutes)...{Style.RESET_ALL}"
            )
            ...
            with open(stdout_log, "w") as stdout_f, open(stderr_log, "w") as stderr_f:
                ...
                whos_pattern = re.compile(
                    r"^\s+[a-zA-Z_]\w*\s+(?:\d+[xX-]\d+(?:[xX-]\d+)*|\d+-D)\s+\d+\s+\w+"
                )
                ...
                while True:
                    ...
                    if process.stdout:
                        ...
                        if line:
                            ...
                            if line_count % 50 == 0:
                                logger.info(
                                    f"{Fore.CYAN}⏳ CAT12 processing... ({line_count} log lines){Style.RESET_ALL}"
                                )

                    # Show a heartbeat every 30 seconds if no output
                    if time.time() - last_output_time > 30:
                        logger.info(
                            f"{Fore.YELLOW}💓 CAT12 still processing...{Style.RESET_ALL}"
                        )
                        last_output_time = time.time()
            ...
            if process.returncode == 0 and not execution_failed:
                ...
                if stderr_log.exists() and stderr_log.stat().st_size > 0:
                    with open(stderr_log, "r") as f:
                        logger.error(
                            f"{Fore.RED}Error output: {f.read()[-500:]}{Style.RESET_ALL}"
                        )
                return False
